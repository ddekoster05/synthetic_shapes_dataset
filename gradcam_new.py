...
def load_model(type):
    ...
    if type == "2D":
        ...
        #checkpoint = torch.load(os.path.join(base_directory, "model_multiview_2.ckpt"))
        checkpoint = torch.load(os.path.join(base_directory, "model_multiview_new_2.ckpt"))
...
def compute_heatmap(model_type, model, image, layer):
    ...
    if model_type == "3D":
        ...
        # 1. Global average pooling over T, H, W
        weights = np.mean(gradients[0], axis=(1, 2, 3))  # (C,)

        # 2. Weighted combination
        cam = np.sum(
            activations[0] * weights[:, np.newaxis, np.newaxis, np.newaxis],
            axis=0
        )  # (T, H, W)

        # 3. Collapse temporal dimension
        cam = np.mean(cam, axis=0)  # (H, W)

    else:
        weights = np.mean(gradients[0], axis=(1, 2))
        cam = np.sum(
            activations[0] * weights[:, np.newaxis, np.newaxis],
            axis=0
        )
...
#image_directory = os.path.join(base_directory, "real_images", "ring_1", "i_2.jpg")
...
#image1_directory = os.path.join(base_directory, "real_images", "cylinder_1", "u_1.jpg")
#image2_directory = os.path.join(base_directory, "real_images", "cylinder_1", "u_2.jpg")
...
image1_directory = os.path.join(base_directory, "edited_images", "cylinder_uninformative_1.png")
image2_directory = os.path.join(base_directory, "edited_images", "cylinder_uninformative_2.png")
